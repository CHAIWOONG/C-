...
int main(void) {
        ...
        // 많은 최적화 문제는 이분 탐색으로 풀 수 있다. 최적화 문제란 어떤 조건(Check(x))을 만족하는 x의 최댓값 또는 최솟값을 찾는 문제를 말한다.
	...
	cout << hi - lo << '\n'; // 2
        ...
        // --------------------------------------------------------- STL unique로 중복 원소 제거 ------------------------------------------------------------------------

        int n;
        int x[1000005];
        vector<int> uni; // unique vector
        ...
        sort(uni.begin(), uni.end()); // sort를 이용해 같은 값을 갖는 원소들이 하나의 구간을 이루도록 만들어준 뒤 적용 ( O(nlogn + n)에 중복 원소를 제거 )
        uni.erase( unique( uni.begin(), uni.end() ), uni.end() ); // 1,2,3,4,5
        
	// unique는 같은 값이 연속해서 나오는 구간을 하나로 압축시켜주는 역할
        // 중복이 제거된 원소들을 앞으로 모아준 후 뒤쪽에는 쓰레기 값들이 들어가게 되는데, 그런 다음 쓰레기 값이 시작되는 구간을 return
        // vector의 erase를 이용해서 뒤쪽을 날리면 중복이 제거된 배열을 얻을 수 있다
	
	// ----------------------------------------------------------- STL unique로 좌표 압축 ------------------------------------------------------------------------
	// 입력값의 범위는 1에서 10^9 정도로 굉장히 큰데 그거를 배열 인덱스처럼 쓰고 싶은 경우 사용
	// 좌표 압축은 배열 v에서 i번째로 작은 수를 i로 바꾸는 식
	// ex) v = [2, 4, -10, 4, -9]라면 압축시킨 뒤의 배열은 v' = [2, 3, 0, 3, 1]
	...
	uni.erase(unique(uni.begin(), uni.end()), uni.end()); // 먼저 중복의 제거를 통해 압축함
	
	for(int i = 0; i < n; i++)
		cout << lower_bound(uni.begin(), uni.end(), x[i]) - uni.begin() << ' '; // 압축된 배열을 바탕으로 인덱스를 크기 순으로 새로 짜서 저장
